#!/usr/bin/env bash
# Trigger the synthetic Store Manager checkout queue scenario from the host.
...
usage() {
  cat <<'EOF'
Usage: store-manager/scripts/simulate-checkout-event.sh [command]

Commands:
  incident                 Inject a checkout queue threshold breach and notify Hermes (default)
  advance-time MINUTES     Advance the clock, measure the checkpoint, and notify Hermes
  reset                    Restore the normal checkout state
  status                   Read the current checkout operating state
EOF
}
...
notify_hermes() {
  ...
  webhook_payload="$(jq -nc \
    ...
    --arg source "synthetic-checkout-vision" \
  ...
  printf '\n%s\n' "==> Posting the checkout event to Hermes at 127.0.0.1:${WEBHOOK_PORT}"
  python3 "$WEBHOOK_PUBLISHER" \
    --route checkout-queue \
...
case "$COMMAND" in
  incident|reset)
    ...
    operation="${COMMAND/incident/queue_surge}"
    payload="$(jq -nc \
    ...
    if [[ "$COMMAND" == "incident" ]]; then
      ...
      WEBHOOK_EVENT_TYPE="checkout_queue_incident"
    fi
    ;;
  advance-time)
    [[ $# -eq 2 && "$2" =~ ^[1-9][0-9]*$ ]] || {
    ...
    NOTIFY_AFTER_EVENT=1
    WEBHOOK_EVENT_TYPE="checkout_queue_checkpoint"
    ;;
  status)
    [[ $# -eq 1 ]] || {
      usage >&2
    ...
    curl --fail --silent --show-error --max-time 10 \
      ...
      "http://${SERVICE_HOST}:${CAMEL_PORT}/v1/store-checkout-operating-state" | jq .
    exit 0
    ;;
  *)
    usage >&2
...
response="$(curl --fail --silent --show-error --max-time 10 \
  ...
  --data "$payload" \
  "http://${SERVICE_HOST}:${CAMEL_PORT}/v1/demo/checkout-events")"
jq '{
  event_result,
  minutes_advanced,
  operating_state: {
