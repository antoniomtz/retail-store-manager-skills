#!/usr/bin/env python3
"""Fetch one bounded Store Manager snapshot from the configured Camel API."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch one normalized Store Manager morning snapshot."
    )
...
def load_config(config_path: Path) -> tuple[str, str, str]:
    try:
        ...
        raise ValueError("the installed Store Manager endpoint configuration is invalid") from error
    ...
    if not isinstance(endpoint, str):
        raise ValueError("the installed Store Manager endpoint is missing")
    ...
    if (
        ...
        or parsed.query
        or parsed.fragment
        or parsed.path != "/v1/store-morning-snapshot"
    ):
        raise ValueError("the installed Store Manager endpoint is outside the supported path")
...
def fetch_snapshot(endpoint: str, store_id: str, business_date: str) -> dict:
    ...
    try:
        ...
        raise RuntimeError("the private Camel snapshot API is unavailable") from error

    if len(payload) > MAX_RESPONSE_BYTES:
        raise RuntimeError("the Camel snapshot exceeded the response-size limit")
    ...
    if not isinstance(snapshot, dict):
        raise RuntimeError("Camel returned an invalid snapshot object")
    if snapshot.get("store_id") != store_id or snapshot.get("business_date") != business_date:
        raise RuntimeError("Camel returned a snapshot for a different store or business date")
    return snapshot
...
def main() -> int:
    ...
    try:
        ...
        print(f"Store Manager snapshot unavailable: {error}", file=sys.stderr)
        return 1
